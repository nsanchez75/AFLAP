...
if [[ -z $rem && -z $Ped ]]; then
	echo -e "Either pedigree file (-P), or progeny individual intermediate data to remove (-r) required"
	echo "$usage"
	exit 1
fi
if [[ -z $thread ]]; then
...
if [[ -z $rem ]]; then
	...
	if [[ $kin == 1 ]]; then
		echo -e "\n\e[31mRunning Kmer kinship\e[0m"
		$DIR/bin/05b_KmerKinship.sh -P $Ped -m $mer
	fi &&
		if [[ -n $Max ]]; then
			echo -e "\n\e[31mDownsampling markers\e[0m"
			$DIR/bin/05c_MarkerReduction.sh -P $Ped -m $mer -U $Max
		fi &&
		echo -e "\n\e[31mBeginning Step 6/6\e[0m" &&
		$DIR/bin/06_ExportToLepMap3.sh -P $Ped -m $mer &&
		if [[ -z $LOD ]]; then
			echo -e "No LOD cutoffs provided, so AFLAP will not run LepMap3. LOD cutoffs can be provided with the -L flag"
		else
