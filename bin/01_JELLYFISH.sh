...
if [[ $JF =~ ^jellyfish ]]; then echo "$JF detected"; else
	echo "jellyfish not detected, please modify your PATH"
	exit 1
fi
...
if [[ $Flab > 0 ]]; then
	echo "Pedigree file specifies individuals which are not F0, F1 or F2. AFLAP has not been validated on this type of data so will terminate."
	exit 1
fi
Ilab=$(awk '{print $1, $2}' $Ped | sort -u | awk '{print $1}' | sort | uniq -c | awk '$1 > 1' | wc -l)
if [[ $Ilab > 0 ]]; then
	echo "Pedigree file specifies the same individual label over multiple generations. This is incompatable with AFLAP so will terminate."
...
PedErr=$(awk '$2 != 0 && NF != 5' $Ped | wc -l)
if (($PedErr > 0)); then
	echo -e "Pedigree file not formatted correctly! Five columns should be present on every progeny line with the fourth and fifth specifying the parents. E.g:\n\t[ProgLabel]\t[1/2]\t[READS]\t[Parent1]\t[Parent2]"
	exit 1
fi
...
for g in $(cat AFLAP_tmp/01/F0.txt); do
	#Check if has has already been created
	if [[ -e AFLAP_tmp/01/ParentalCounts/$g.jf${mer} ]]; then
		#If so, skip.
		...
		for r in $Reads; do if [[ ! -e $r ]]; then
			echo -e "file $r not found, terminating"
			exit 1
		fi; done
		jellyfish count -s 1G -t $thread -m $mer -C -o AFLAP_tmp/01/ParentalCounts/$g.jf${mer} <(zcat $Reads) &&
			...
			if [[ ! -f AFLAP_tmp/01/ParentalCounts/$g.jf${mer} ]]; then
				echo -e "JELLYFISH for $g did not complete. Is the file gzipped? Exiting"
				exit 1
			fi
	fi
done
...
for g in $(cat AFLAP_tmp/01/Prog.txt); do
	#Check if has has already been created
	if [[ -e AFLAP_tmp/01/ProgCounts/$g.jf${mer} ]]; then
		#If so, skip.
