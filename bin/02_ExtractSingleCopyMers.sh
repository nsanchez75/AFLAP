...
if [[ $JF =~ ^jellyfish ]]; then echo -e "\n$JF detected" >&2; else
	echo -e "\njellyfish not detected, please modify your PATH" >&2
	exit 1
fi
...
for g in $(cat AFLAP_tmp/01/LA.txt); do
	if [[ -e AFLAP_tmp/02/ParentalHisto/$g.${mer}.histo ]]; then #1
	...
	if [[ $BC == 1 ]]; then #1
		...
		Hom=0
		Het=0
		if (($HomPeak > $HomLo && $HomTest < $HomUp)); then #2
			echo "Additional Hom peak detected at $HomPeak"
			Hom=1
		fi                                                  #2
		if (($HetPeak > $HetLo && $HetPeak < $HetUp)); then #2
			echo "Additional Het peak detected at $HetPeak"
			Het=1
		fi                                    #2
		if [[ $Hom == 1 && $Het != 1 ]]; then #2
