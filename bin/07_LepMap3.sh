...
if [[ -z $Ped || -z $LOD || -z $mer ]]; then
	echo "One or more options not set. Are you trying to run this outside of the AFLAP control script?"
	echo "$usage"
	exit 1
fi
if [[ -z $Threads ]]; then
...
if [[ -e AFLAP_tmp/01/LA.txt && -e AFLAP_tmp/02/Boundaries.txt ]]; then
...
if [[ ! -e $DIR/../ThirdParty/LepMap3/bin/SeparateChromosomes2.class ]]; then
...
for g in $(cat AFLAP_tmp/01/LA.txt); do
	if [[ -e AFLAP_tmp/03/${g}_CrossedTo.txt ]]; then
	...
	if [[ ! -f AFLAP_Results/${g}_m${mer}_L${Lo}_U${Up}_${P0}.ForLepMap3.tsv ]]; then
		echo "Genotype table is missing. Please rerun AFLAP.sh to generate"
		exit 1
	else
		echo -e "Initiating LepMap3 for $g\n"
		if [[ -f AFLAP_Results/LOD${LOD}/${g}_m${mer}_L${Lo}_U${Up}_${P0}.LOD${LOD}.txt ]]; then
			echo "Previous results detected. Skipping Seperate Chromosomes"
		else
		...
		if (($Threads > $LGcou)); then
			echo -e "Running linkage group ordering in parallel as number of threads exceeds number of linkage groups (Maximum efficiency!)"
