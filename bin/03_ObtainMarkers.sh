...
if [[ -e AFLAP_tmp/01/Crosses.txt ]]; then
...
if [[ -e AFLAP_tmp/02/Boundaries.txt ]]; then
...
if [[ $AB =~ ^ABYSS ]]; then echo "$AB detected"; else
	echo "ABySS not detected, please modify your PATH"
	exit 1
fi
...
for g in $(cat AFLAP_tmp/01/LA.txt); do
	...
	if [[ -e AFLAP_tmp/02/ParentalHisto/${g}_m${mer}_L${Lo}_U${Up}.fa ]]; then
	...
	if [[ -e AFLAP_tmp/03/ParentalMarkers/${g}_m${mer}_MARKERS_L${Lo}_U${Up}_$P0.fa ]]; then
		...
		for f in $(cat AFLAP_tmp/03/${g}_CrossedTo.txt); do
			# Identify opposing parental hashes.
			if [[ -e AFLAP_tmp/01/ParentalCounts/$f.jf${mer} ]]; then
